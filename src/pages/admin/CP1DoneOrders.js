...
function CP1DoneOrders() {
                ...
                <ModalContent maxH={'500px'} overflowY='scroll'>
                    ...
                    <ModalBody>
                        <Table marginTop={2} variant='simple' size="sm">
                            ...
                            <Tbody>
                                {(qcs.length === 0) ? <></> : qcs.map(
                                    (qc, index) => <Tr key={qc.id}>
                                    ...
                                    (expert, index) => <Tr key={expert._id}>
                                        ...
                                        <Td>
                                            <Button onClick={async () => {
                                                ...
                                                try {
                                                    const response = await axios.post(apiUrl + '/expert/assignment/ask',
                                                        {
    ...
    async function _fetchAssignments() {
        try {
            ...
            const response = await axios.get(apiUrl + '/assignment/fetch?status=CP1%20Done', config);
                        ...
                        {assignments.map(
                            (assignment, index) => <Tr key={assignment.id}>
                                ...
                                <Td>
                                    <HStack>
                                        <Button
                                            display={(localStorage.getItem('userRole') === "Super Admin" || localStorage.getItem("userRole") === "Operator" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                            onClick={async () => openQcModal(index)}>Assign QC</Button>
                                        <Button
                                            display={(localStorage.getItem('userRole') === "Super Admin" || localStorage.getItem("userRole") === "Operator" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                            onClick={async () => openExpertModal(index)}>Assign Expert</Button>
                                        <Button
